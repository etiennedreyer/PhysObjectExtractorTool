...
private:
  ...
  std::vector<float> electron_pfreliso03all;
  std::vector<float> electron_pfreliso04all;
...
ElectronAnalyzer::ElectronAnalyzer(const edm::ParameterSet &iConfig) {
  ...
  mtree->Branch("electron_pfreliso03all", &electron_pfreliso03all);
  mtree->GetBranch("electron_pfreliso03all")
      ->SetTitle("electron particle flow relative isolation cone 03");
  mtree->Branch("electron_pfreliso04all", &electron_pfreliso04all);
  mtree->GetBranch("electron_pfreliso04all")
      ->SetTitle("electron particle flow relative isolation cone 04");
  ...
  mtree->GetBranch("electron_dxy")
      ->SetTitle("electron transverse plane impact parameter (mm)");
...
float ElectronAnalyzer::effectiveArea0p3cone(float eta) {
  if (fabs(eta) < 1.0)
    return 0.13;
...
void ElectronAnalyzer::analyze(const edm::Event &iEvent,
  ...
  electron_phi.clear();
  electron_ch.clear();
  electron_pfreliso03all.clear();
  electron_pfreliso04all.clear();
  // electron_iso.clear();
  ...
  if (myelectrons.isValid()) {
    ...
    for (reco::GsfElectronCollection::const_iterator itElec =
             myelectrons->begin();
         itElec != myelectrons->end(); ++itElec) {

      // int missing_hits =
      //     itElec->gsfTrack()->trackerExpectedHitsInner().numberOfHits() -
      //     itElec->gsfTrack()->hitPattern().numberOfHits();
      // bool passelectronveto = !ConversionTools::hasMatchedConversion(
      //     *itElec, hConversions, beamspot.position());
      ...
      if (itElec->passingPflowPreselection()) {
        ...
        electron_pfreliso03all.push_back(
            (iso03.chargedHadronIso +
             std::max(0.0,
                      iso03.neutralHadronIso + iso03.photonIso - rho * Aeff)) /
            itElec->pt());
        auto iso04 = itElec->pfIsolationVariables();
        electron_pfreliso04all.push_back(
            (iso04.chargedHadronIso +
             std::max(0.0,
                      iso04.neutralHadronIso + iso04.photonIso - rho * Aeff)) /
            itElec->pt());
        // el_pfIso = (iso03.chargedHadronIso +
        //             std::max(0.0, iso03.neutralHadronIso + iso03.photonIso -
        //                               rho * Aeff)) /
        //            itElec->pt();
      }
      else{
        electron_pfreliso03all.push_back(-999);
        electron_pfreliso04all.push_back(-999);
      }
      ...
      //       abs(1 / itElec->ecalEnergy() -
      //           1 / (itElec->ecalEnergy() / itElec->eSuperClusterOverP())) <
      //           .05 &&
      //       el_pfIso < .15) {
      ...
      //   if (abs(itElec->deltaEtaSuperClusterTrackAtVtx()) < .009 &&
      //       abs(itElec->deltaPhiSuperClusterTrackAtVtx()) < .1 &&
      ...
      //     if (abs(itElec->deltaEtaSuperClusterTrackAtVtx()) < .007 &&
      //         abs(itElec->deltaPhiSuperClusterTrackAtVtx()) < .03 &&
      //         abs(trk->dz(pv)) < .1) {
      //       isMedium = true;

      //       if (abs(itElec->deltaEtaSuperClusterTrackAtVtx()) < .005 &&
      //           abs(itElec->deltaPhiSuperClusterTrackAtVtx()) < .02 &&
      //           missing_hits <= 0 && el_pfIso < .10) {
      //         isTight = true;
      //       }
      //     }
...
void ElectronAnalyzer::beginRun(edm::Run const &, edm::EventSetup const &) {}

// ------------ method called when ending the processing of a run  ------------
void ElectronAnalyzer::endRun(edm::Run const &, edm::EventSetup const &) {}
// ------------ method called when starting to processes a luminosity block
// ------------
void ElectronAnalyzer::beginLuminosityBlock(edm::LuminosityBlock const &,
                                            edm::EventSetup const &) {}

// ------------ method called when ending the processing of a luminosity block
// ------------
void ElectronAnalyzer::endLuminosityBlock(edm::LuminosityBlock const &,
                                          edm::EventSetup const &) {}

// ------------ method fills 'descriptions' with the allowed parameters for the
// module  ------------
void ElectronAnalyzer::fillDescriptions(
